...
function renderQualAudits() {
  ...
  tb.innerHTML = list.map(a => `<tr>
    ...
    <td><div class="act-btns">
      ...
      ${hasPerm('report_delete_audits') ? `<button class="btn btn-danger btn-sm" onclick="deleteQualAudit('${a.id}')"><i class="ti ti-trash"></i></button>` : ''}
    </div></td>
  </tr>`).join('');
}
...
function openQualAuditModal() {
  ...
  // n'importe quel rôle peut désormais se voir accorder ces droits
  // individuellement, pas seulement 'admin'/'non-collaborateur'.
